...
def filter(sc_mock: SimplicialComplex):
    return Filter(sc_mock)


class TestFilter:

    def test_get_true_signal_gradient(
        self, filter: Filter, f_mock: np.ndarray
    ):
        component = "gradient"
    ...
    def test_calcualte_error(
        self, filter: Filter, f0_mock: np.ndarray, f_mock: np.ndarray
    ):
        excepted_error = 0.46
        error = filter.calculate_error(f_mock, f0_mock)
        assert np.isclose(error, excepted_error, atol=1e-2)

    def test_power_iteration_algorithm(
        selfl, filter: Filter, sc_mock: SimplicialComplex
    ):
